...
<script>
	...
	function initialize() {
	    ...
	    rs_distance = String(calcDistance(p1, p2));
 	    var support_now_info = support_id +"("+rs_distance+"km)";
...
/* 	function matching_update(resp){
		...
		$("#supp_y").val(resp.user_y);
 		...
 		var mulat = Number(resp.user_x);
		var mulng = Number(resp.user_y);
	...
	function matching_update(resp){
		...
		//var center = {((rlat + mulat)/2) , ((rlng + mlng)/2)} 
		
		deleteMarkers();
		addMarker(rlocation, ricon);
		addMarker(mulocation, sicon);
		map.setCenter(mulocation);
	...
	function waitList(resp){
		...
		$.each(resp, function(index, item){
			tag += '<option value=';
			tag += '"'+item.userid+'">';
			tag += item.userid + '(' + item.inter_distance + 'km)'; 
			tag += '</option>';
		});
...
<%--  						<c:forEach var="suppList" items="${suppList}">
								<option value="${suppList.userid}">${suppList.userid}</option>
							</c:forEach>
--%>
						</select>
					</div>
